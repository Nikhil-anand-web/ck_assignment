...
const MainModule = ({ cats }) => {
    const rtr = useRouter()

    const onStatusFlip = async (id) => {
        try {
            const res = await axios.post('/api/v1/admin/categoryStatusToggle', { id });
            if (res.data.success) {
    ...
    const onDelete = async (id) => {
        ...
        try {
          const res = await axios.delete(`/api/v1/admin/deleteCategory`, {
            data: [{ id }]
          });
          if (res.data.success) {
            toast.success(res.data.message)
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem onClick={()=>rtr.push(`packages/${cat.id}`)} >View Products</DropdownMenuItem>
                                        <DropdownMenuItem onClick={()=>rtr.push(`edit-category/${cat.id}`)}>Edit</DropdownMenuItem>
